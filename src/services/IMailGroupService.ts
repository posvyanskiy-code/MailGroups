...
export interface IMailGroupService {
  ...
  approveJoinRequestInGroup(groupId: string, reqId: string): Promise<void>;
  rejectJoinRequestInGroup(groupId: string, reqId: string): Promise<void>;
  ...
  // Mail
  sendMail(groupId: string, subject: string, bodyHtml: string): Promise<void>;
  getMailHistory(groupId: string): Promise<Array<{ id: string; subject: string; recipientCount: number; status: string; createdAt: string; sentAt?: string }>>;
}
